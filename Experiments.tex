...
\paragraph{Mismatch measurement.}\label{sec:exp_measurement}
We measure $(k_t,p_t)$ on Qwen1.5-MoE-A2.7B-Chat (24 sparse layers, 60 experts, top-4 routing) and, as a dense control, on Qwen1.5-14B-Chat. For 2{,}500 DAPO-Math prompts we sample eight responses each with vLLM at temperature one, record the log-probability of every sampled token at the sampling step, and rescore the same sequences with the training engine at the same parameters, which gives 12.3M MoE tokens and 12.4M dense tokens together with the expert routes on both sides. The empirical mean of $k_t$ is $1.0000$ on both architectures. Measurement controls, the paired stale-snapshot measurement behind Figure~\ref{fig:two_channel}, and the training-time logging are described in Appendix~\ref{app:experimental-diagnostics}.

\paragraph{Training.}
We train Qwen1.5-MoE-A2.7B-Chat with the official AReaL implementation on the GSM8K training set (SGLang rollouts, FSDP training, both bf16) and report greedy exact-match accuracy on the 1{,}319 held-out test problems, for which one arm has a standard error of about $1.3$ points. All arms share one recipe and one code path (Appendix~\ref{app:experimental-implementation}); they differ only in the ratio operator of \Cref{sec:prelim_operator}: no correction ($M\equiv1$), the exact ratio ($M=k$), TIS with cap $2$, IcePop with mask $[0.5,5]$, the binary-KL mask of AReaL (KPop) with threshold $2$, and CIS (\Cref{alg:cis}) with $\lambda_+=2.3$ and $\kappa=5\times10^{-3}$. Each method is run with three seeds, and every method is run once more with the SGLang KV cache quantized to FP8. Held-out accuracy at the final checkpoint is the arbiter of every comparison; training reward is reported but is not used to choose between methods.
