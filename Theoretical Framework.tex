...
Both parts of \Cref{def:logit_displacement} follow from where the mismatch is
produced. The two engines differ only before the softmax: write
$\mathbf z\in\mathbb{R}^{V}$ for the pre-softmax logit vector of the training
engine at the prefix $h_t$, with $V$ the vocabulary size, so that
\begin{equation}
...
\end{equation}
where $\boldsymbol\delta\in\mathbb{R}^{V}$ is the logit perturbation of the
inference engine relative to the training engine, produced by floating-point
rounding, reduction order, and, on a mixture of experts, by routing
disagreement. Write $w_j:=e^{z_j}\big/\sum_{j'\ne y_t}e^{z_{j'}}$ for the
off-target softmax weights of $\mathbf z$.

\begin{lemma}
\label{lem:displacement}
Under \eqref{eq:engine_logits}, the pair $(p_t,q_t)$ satisfies
\eqref{eq:logit_displacement} with
\begin{equation}
\varepsilon_t
=
-\delta_{y_t}
...
\begin{remark}
\label{rem:why_logodds}
\Cref{lem:displacement} accounts for both parts of
\Cref{def:logit_displacement}. First, $p_t$ and $q_t$ are each normalized by a
sum over the whole vocabulary, and that sum cancels in $p_t/(1-p_t)$, leaving
$\logit p_t=z_{y_t}-\log\sum_{j\ne y_t}e^{z_j}$; a perturbation that is additive
on the logits is therefore still additive in this coordinate, which is why the
displacement appears in the log-odds and not in the probabilities or their
ratio. Second, the right-hand side of \eqref{eq:eps_identity} involves only
$\boldsymbol\delta$ and the off-target weights, and not $z_{y_t}$, whereas
raising or lowering $z_{y_t}$ alone takes $p_t$ across $(0,1)$. Independence
therefore requires only that the law of $\boldsymbol\delta$ not vary with the
confidence, which is what \Cref{fig:eps_dist} tests.
\end{remark}
...
the gradient of the surrogate term in \eqref{eq:objective}; the mismatch ratio
$k_j$ is the importance sampling factor that multiplies it, rewriting a sample
drawn from the inference engine as an expectation under the training engine.
Since $k_j$ does not depend on $\theta$, the gradient of the objective and its
$n$-sample estimator are
\begin{equation*}
G_0:=\nabla_\theta\mathcal J=\E[k\,\ell'],
\qquad
\widehat G:=\frac{1}{n}\sum_{j=1}^{n}k_j\,\ell'_j ,
\end{equation*}
where the expectation is over the randomness of the data used for one update:
$x\sim\mathcal D$, the rollout drawn from
$\pi_{\mathrm{infer}}(\cdot\mid x;\theta_{\mathrm{old}})$, and the numerical
discrepancy between the two engines on each token. We measure the quality of the
estimate by the mean-squared error $\E\big\|\widehat G-G_0\big\|_2^2$.
...
Separating the contribution of $\varepsilon_t$ into a scalar that carries no
confidence is exactly what the independence of \Cref{def:logit_displacement}
buys. \Cref{fig:mgf} shows $\E[e^{\alpha\varepsilon_t}]$ computed from the
measured displacements; details are given in \Cref{app:mgf}. At $\alpha=1$ it is
only $7.07$, whereas at $\alpha=2$ we obtain
...
\eqref{eq:exact_risk} is of order $10^{3}$. One of the reasons of this explosive
variance is the explosion of the importance sampling factor $k_j$ when the
inference engine and the training engine disagree significantly. The other side
is harmless: by \eqref{eq:k_geometry} the ratio satisfies $k_t\ge p_t$, so small
values of $k_j$ are bounded and contribute no variance.
...
some bias in exchange for a smaller variance so that their sum is reduced. To
mitigate this explosive variance, many existing works introduce some bias in
exchange for a well-conditioned variance, especially by downweighting the
suspicious (overly large) importance sampling factor $k_j$ with the knowledge of
its likelihood $p_j$: TIS caps $k_t$ \citep{cite}, IcePop masks tokens falling
outside an interval \citep{cite}, and KPop masks by a KL threshold
\citep{cite}, all of which attenuate a weight but never amplify it, and which
differ only in the rule that decides how much to attenuate. Systematically, we
can formulate their estimator as
\begin{equation}
\widehat G_{f}
:=\frac{1}{n}\sum_{j=1}^{n}f(k_j,p_j)\,\ell'_j ,
\qquad
f\in\mathcal F
:=\big\{f:\ 0\le f(k,p)\le k\big\},
\label{eq:operator_class}
\end{equation}
where $f(k_j,p_j)$ is the corrected importance sampling factor obtained with the
likelihood $p_j$, satisfying $f(k,p)\in[0,k]$, which implies that it cannot
exceed the original importance sampling factor.

The restriction to $\mathcal F$ agrees with the methods above: since the
variance comes entirely from the upper side, amplifying a weight would only
increase the bias and the variance at once. \Cref{app:no_lower} and the
dose--response study of \Cref{app:ablations} give further theoretical and
empirical support for this restriction.
% \subsection{Why a correction is needed}
% \label{sec:why_correction}
...
% once.
\subsection{The optimal correction}
\label{sec:optimal_operator}

For independent token contributions the mean-squared error of $\widehat G_{f}$
splits into a squared bias and a variance term, which gives the upper bound
\begin{equation}
\E\big\|\widehat G_{f}-G_0\big\|_2^2
\;\le\;
\underbrace{\big\|\E\big[f(k_t,p_t)\,\ell'_t\big]-G_0\big\|_2^2}_{\text{squared bias}}
+\underbrace{\frac{1}{n}\,\E\big[f(k_t,p_t)^2\,\|\ell'_t\|_2^2\big]}_{\text{variance}}
\;=:\;
\mathcal R(f).
\label{eq:surrogate_risk}
\end{equation}
Equation \eqref{eq:surrogate_risk} turns the trade-off of
\Cref{sec:why_correction} into something that can be minimized: the first term
grows as $f$ departs from $k_t$, and the second shrinks as $f$ is attenuated,
since $f\le k_t$ keeps it below the corresponding quantity for the exact ratio.
We take $\mathcal R$ as the surrogate risk and minimize it over $\mathcal F$.

By \eqref{eq:k_geometry}, $k_t-1=(1-p_t)(e^{\varepsilon_t}-1)$ with the law of
$e^{\varepsilon_t}$ free of the confidence, so the dispersion of $k_t$ is
proportional to $1-p_t$: on a high-confidence token the ratio should already sit
close to one, and a large deviation there is more likely to come from numerical
noise, whereas on a low-confidence token the ratio is dispersed to begin with
and clipping it too tightly discards useful gradient signal. The admissible band
should therefore widen with $1-p_t$, and minimizing $\mathcal R$ returns exactly
such a band.

\begin{theorem}[CIS, informal]
\label{thm:cis_informal}
Under \Cref{def:logit_displacement},
\begin{equation}
f^{\star}(k_t,p_t)
=
\argminop_{f\in\mathcal F}\mathcal R(f)
=
...
The factor $1-p_t$ in the band comes directly from
\Cref{def:logit_displacement}: precisely because the law of $\varepsilon_t$ does
not depend on the confidence, the factor $1-p_t$ of \eqref{eq:k_geometry} is the
only source of confidence dependence in the dispersion of $k_t$, so the optimal
band must follow it, and no threshold that ignores $p_t$ can fit both ends at
once.

\begin{theorem}[Informal]
\label{thm:cis_risk}
Under the correction of \Cref{thm:cis_informal},
\begin{equation}
\E\big\|\widehat G_{f^{\star}}-G_0\big\|_2^2
\;\le\;
...
variance is replaced by $\E[\min(e^{\varepsilon_t},1+\lambda)^2]$. Pointwise,
\begin{equation*}
\min(e^{\varepsilon_t},1+\lambda)^2
\;\le\;
(1+\lambda)\min(e^{\varepsilon_t},1+\lambda)
\;\le\;
(1+\lambda)\,e^{\varepsilon_t},
\end{equation*}
and taking expectations gives
$\E[\min(e^{\varepsilon_t},1+\lambda)^2]\le(1+\lambda)\,\mu$. The exponent drops
from two to one: the explosive second exponential moment is traded for the
first, the constant $\mu$ of \Cref{def:logit_displacement}, which reads $7.07$
off \Cref{fig:mgf} at $\alpha=1$ and is both finite and estimable. The price is
the bias term, set by the exceedance that the correction removes and decreasing
in $\lambda$.
\end{remark}
...
The correction adds one elementwise pass and no forward or backward computation.
